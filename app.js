...
// Helper para obtener recetas de localStorage con manejo de errores
function getRecetasFromStorage() {
  try {
    return JSON.parse(localStorage.getItem('recetas') || '[]');
  } catch (e) {
    console.error('Error al parsear las recetas desde localStorage', e);
...
function guardarReceta() {
  ...
  const recetas = getRecetasFromStorage();
  recetas.push(receta);
...
function cargarRecetas(recetasParam) {
  const recetas = recetasParam || getRecetasFromStorage();
  ...
  if (recetas.length === 0) {
    ...
    renderizarRecetas(recetas, lista);
  }
}
...
// Renderizar una lista de recetas en el DOM
function renderizarRecetas(recetas, contenedor) {
...
document.addEventListener('DOMContentLoaded', () => {
  // Navegación
  document.querySelector('button[data-seccion="inicio"]').onclick = () => mostrar('inicio');
  document.querySelector('button[data-seccion="crear"]').onclick = () => mostrar('crear');
  ...
  document.getElementById('buscar').addEventListener('input', function () {
    ...
    cargarRecetas(resultados); // Reutilizamos cargarRecetas para mostrar resultados
  });
